...
void videowidget::initUi()
{
    ...
    //    screenHeigh=list_screen.at(0)->geometry().height();
        ...
        //pbt1->setFlat(true);
        connect(pbt1, &QPushButton::clicked, this, &videowidget::pbclose);
        ...
        QFile file(":/videiowidget/style.qss");
...
void videowidget::mediaPlayerInit()
{
...
void videowidget::btn_play_clicked()
{
...
void videowidget::btn_next_clicked()
{
    videoPlayer->stop();
...
void videowidget::btn_volmeup_clicked()
{
    /* 点击每次音量+5 */
...
void videowidget::btn_fullscreen_clicked()
{
    /* 全屏/非全屏操作 */
    vWidget[0]->setVisible(!pushButton[4]->isChecked());
}

void videowidget::btn_volmedown_clicked()
{
    /* 点击每次音量-5 */
    volumeSlider->setValue(volumeSlider->value() - 5);
    videoPlayer->setVolume(volumeSlider->value());
...
void videowidget::mediaPlayerStateChanged(
        QMediaPlayer::State
        state)
{
    switch (state) {
...
void videowidget::listWidgetCliked(QListWidgetItem *item)
{
    videoPlayer->stop();
...
void videowidget::mediaPlaylistCurrentIndexChanged(
        int index)
{
    if (-1 == index)
        return;
...
void videowidget::musicPlayerDurationChanged(
        qint64 duration)
{
...
void videowidget::mediaPlayerPositionChanged(
        qint64 position)
{
...
void videowidget::durationSliderReleased()
{
    /* 设置媒体播放的位置 */
    videoPlayer->setPosition(durationSlider->value() * 1000);
}

void videowidget::volumeSliderReleased()
{
    /* 设置音量 */
    videoPlayer->setVolume(volumeSlider->value());
}

void videowidget::scanVideoFiles()
{
