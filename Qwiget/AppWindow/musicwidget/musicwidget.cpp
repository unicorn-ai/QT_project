    ...
    : QMainWindow(parent)
{
    ...
    QFile file(":/musicwidget/style.qss");
    ...
    musicinit();
    pbt1 = new QPushButton(this);
    pbt1->setGeometry(0,0,80,40);
    setBackground(QPixmap(":/misc/resource/image/back1.jpg"));
    connect(pbt1, &QPushButton::clicked, this, &musicwidget::pbclose);



}

void musicwidget::pbclose()
{
    setBackground(QPixmap(":/misc/resource/image/back.jpg"));
    this->close();
    //terminate();
    this->deleteLater();
}

void musicwidget::setBackground(const QPixmap &pixmap)
{
    QPalette  palette = this->palette();
    palette.setBrush(backgroundRole(), pixmap.scaled(800, 480, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
    setPalette(palette);
    setAutoFillBackground(true);
}

void musicwidget::musicLayout()
{
    ...
    QImage Image;
    Image.load(":/musicwidget/images/cd.png");
...
void musicwidget::btn_play_clicked()
{
...
void musicwidget::btn_next_clicked()
{
    musicPlayer->stop();
...
void musicwidget::btn_previous_clicked()
{
    musicPlayer->stop();
    int count = mediaPlaylist->mediaCount();
    if (0 == count)
...
void musicwidget::mediaPlayerStateChanged(
        QMediaPlayer::State
        state)
{
    switch (state) {
...
void musicwidget::mediaPlaylistCurrentIndexChanged(
        int index)
{
    if (-1 == index)
        return;
...
void musicwidget::musicPlayerDurationChanged(
        qint64 duration)
{
...
void musicwidget::mediaPlayerPositionChanged(
        qint64 position)
{
...
void musicwidget::durationSliderReleased()
{
    /* 设置媒体播放的位置 */
    musicPlayer->setPosition(durationSlider->value() * 1000);
}

void musicwidget::scanSongs()
{
...
void musicwidget::mediaPlayerInit()
{
